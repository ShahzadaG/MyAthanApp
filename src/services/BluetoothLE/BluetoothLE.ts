import {
  ...
  class BLEServiceInstance {
    ...
    scanDevices = async (onDeviceFound: (device: Device) => void, UUIDs: UUID[] | null = null, legacyScan?: boolean) => {
      this.manager
        .startDeviceScan(UUIDs, { legacyScan, allowDuplicates: false }, (error, device) => {
          if (error) {
            this.onError(error)
    ...
    connectToDevice = (deviceId: DeviceId) =>
      new Promise<Device>((resolve, reject) => {
        ...
        this.manager
          ...
          .then((device) => {
            this.device = device;
            return this.discoverAllServicesAndCharacteristicsForDevice(); // Discover services and characteristics
          })
          .then((device) => {
            resolve(device);
          })
